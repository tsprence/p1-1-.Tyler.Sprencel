...
alarmW, alarmH, callX, callY, callW, callH, yesX, yesY,
yesW, yesH, noX, noY, noW, noH;
boolean circleOver, circleOver1, circleOver2, circleOverF4,
circleOverF5, circleOverF6, overOpenDoors, overCloseDoors, 
overHandicap, overAlarm, overCall, overAcknowledge, overDecline = false;
boolean pressedF1, pressedF2, pressedF3, pressedF4, pressedF5, pressedF6, 
pressedOpen, pressedClose, pressedHandicap, pressedAlarm, pressedCall, 
pressedAcknowledge, pressedDecline = false;
int circleSize = 93;
//Color variables
...
alarmColor, callColor, yesColor, noColor;
//Font variable
PFont f;
//Timer variables
...
void setup() {
    ...
    //Setting time checking variable to current time
...
void draw() {
    ...
    if(pressedF1) {
       ...
       if(millis() > lastTimeChecked + timeInterval) {
         lastTimeChecked = millis();
         ding.play();
       }
    } else if(pressedF3) {
       ...
       rect(100, 50, 600, 100, 100);
       //background(150); 
       if (overAcknowledge && pressedAcknowledge != true) {
          fill(circleHighlight);
       } else {
          fill(yesColor);
       }
       rect(yesX, yesY, yesW, yesH, 100);
       if (overDecline && pressedDecline != true) {
          fill(circleHighlight);
       } else {
          fill(noColor);
       }
       rect(noX, noY, noW, noH, 100);
       
       fill(0);
       textSize(18);
       text("Yes, I Meant To Press This", 155, 110);
       text("No, Take Me Back", 475, 110);
    } else if(pressedCall) {
         fill(255);
         text("Call Initiated Please Wait", 280, 110);
    } else if(pressedAcknowledge) {
         fill(255);
         text("Alarm Activated", 280, 110);
    } else if(pressedDecline) {
         fill(255);
         text("Coward...", 280, 110);
    }
    //Font of text
...
void mousePressed() {
  if (circleOver) {
    ...
    pressedAlarm = false;
    pressedCall = false;
    pressedAcknowledge = false;
    pressedDecline = false;
  }
  else if(circleOver1) {
    background(200);
...
void update(int x, int y) {
  if (overCircle(circleX, circleY, circleSize) ) {
    ...
    overDecline = false;
  ...
  else if (overAcknowledge(yesX, yesY, yesW, yesH)){
     circleOver2 = false;
     circleOver = false;
     circleOver1 = false;
     circleOverF4 = false;
     ...
     overAlarm = false;
     overCall = false;
     overAcknowledge = true;
     overDecline = false;
  }
  else if (overDecline(noX, noY, noW, noH)){
     circleOver2 = false;
     circleOver = false;
     circleOver1 = false;
     circleOverF4 = false;
...
boolean overAcknowledge(int x, int y, int width, int height)  {
  if (mouseX >= x && mouseX <= x+width && 
      mouseY >= y && mouseY <= y+height) {
    return true;
  } else {
...
boolean overDecline(int x, int y, int width, int height)  {
  if (mouseX >= x && mouseX <= x+width && 
      mouseY >= y && mouseY <= y+height) {
    return true;
  } else {
